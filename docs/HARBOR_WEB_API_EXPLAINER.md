...
Harbor exposes two global JavaScript APIs to web pages for AI capabilities:
...
These APIs enable web applications to leverage AI capabilities while maintaining user control over permissions and resource access. The implementation uses a browser extension that acts as a bridge between web pages and either local AI models (Ollama, llamafile) or cloud providers.
...
When Harbor is installed, three global objects are exposed:
...
window.addEventListener('harbor-provider-ready', (event) => {
  console.log('Providers available:', event.detail.providers);
  // { harbor: true, chrome: false }
});
...
The `window.harbor` namespace provides guaranteed access to Harbor APIs without potential conflicts with Chrome's built-in AI.

```typescript
window.harbor = {
  ai: AIApi;          // Harbor's AI API (same as window.ai if no Chrome AI)
  agent: AgentApi;    // Agent API (same as window.agent)
...
1. **Local-First**: Default configuration uses local LLM (Ollama). No data leaves the machine.

2. **No Telemetry**: Harbor does not collect usage data, analytics, or crash reports.

3. **No Payload Logging**: Tool arguments and results are not logged by default.

4. **User Data Control**: 
   - Users control which AI backend is used
   - Users can use local-only models
   - Session data is not persisted beyond the session

5. **Credential Isolation**: API keys and secrets are stored with restricted file permissions (600).
...
The implementation uses a multi-process architecture:
...
Harbor's `window.ai` API is designed for compatibility with Chrome's built-in Prompt API.

### Compatibility

| Feature | Chrome Prompt API | Harbor |
|---------|-------------------|--------|
| `ai.languageModel.create()` | ✓ | ✓ |
| `ai.languageModel.capabilities()` | ✓ | ✓ |
| `session.prompt()` | ✓ | ✓ |
...
Harbor extends the Chrome API with:

| Feature | Description |
|---------|-------------|
| `window.agent` | Tool calling, browser access, agent runs |
| `window.harbor` | Guaranteed namespace |
| Provider selection | Choose Chrome vs. Harbor backend |

### Unified Usage

```javascript
// Works with both Chrome and Harbor
const session = await window.ai.languageModel.create({
  systemPrompt: 'Be helpful'
});
const response = await session.prompt('Hello');

// Harbor-specific: choose provider explicitly
const harborSession = await window.ai.createTextSession({
  provider: 'harbor',  // Force Harbor backend
  systemPrompt: 'Be helpful'
});

// Harbor-specific: agent capabilities
...
*This document is a draft proposal for web AI APIs. Implementations should refer to the source code for authoritative behavior.*
